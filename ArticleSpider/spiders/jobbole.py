...
class JobboleSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        title = response.css(".enctry-header h1::text").extract()[0]
        ...
        try:
            create_data = datetime.datatime.strptime(create_data, "%Y/%m/%d").date()
        except Exception as e:
            create_data = datetime.datetime.now().date()
        article_item["create_date"] = create_data
        article_item["front_image_url"] = [front_image_url]
        article_item["parse_nums"] = praise_nums
        article_item["comment_nums"] = comment_nums
        article_item["fav_nums"] = fav_nums
        article_item["tags"] = tags
        article_item["content"] = content
        yield article_item.get_insert_sql()
